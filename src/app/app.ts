...
import { cleanCivilIso, shiftCivilMinutes } from './weather-scene';

type WeatherView = 'now' | 'forecast';
type LabelKey = keyof typeof LABELS;
const LABELS = {
  search: ['Ricerca', 'Search'],
  title: ['Controlla il meteo della tua città', 'Check the weather in your city'],
  subtitle: [
    'Condizioni attuali e previsioni affidabili, in pochi secondi.',
    'Current conditions and reliable forecasts in seconds.',
  ],
  city: ['Città', 'City'],
  wait: ['Attendi...', 'Please wait...'],
  searchButton: ['Cerca', 'Search'],
  searching: ['Ricerca località...', 'Searching locations...'],
  recent: ['Recenti', 'Recent'],
  clear: ['Cancella', 'Clear'],
  favorites: ['Preferiti', 'Favourites'],
  suggested: ['Città suggerite', 'Suggested cities'],
  legal: ['Privacy, fonti e licenze', 'Privacy, sources and licences'],
  changeCity: ['Cambia città', 'Change city'],
  ready: ['Pronto per la ricerca', 'Ready to search'],
  emptyTitle: [
    'Cerca una città per visualizzare il meteo.',
    'Search for a city to see the weather.',
  ],
  emptyCopy: [
    'Troverai condizioni attuali, prossime 24 ore e previsioni per 7 giorni.',
    'You will find current conditions, the next 24 hours and a 7-day forecast.',
  ],
  location: ['Località', 'Location'],
  now: ['Adesso', 'Now'],
  forecast: ['Previsioni', 'Forecast'],
  humidity: ['Umidità', 'Humidity'],
  wind: ['Vento', 'Wind'],
  localTime: ['Ora locale', 'Local time'],
  sunrise: ['Alba sole', 'Sunrise'],
  sunset: ['Tramonto sole', 'Sunset'],
  moonrise: ['Alba luna', 'Moonrise'],
  moonset: ['Tramonto luna', 'Moonset'],
  updated: ['Meteo aggiornato', 'Weather updated'],
  hourly: ['Prossime 24 ore', 'Next 24 hours'],
  daily: ['Prossimi 7 giorni', 'Next 7 days'],
  rainChance: ['Probabilità pioggia', 'Rain chance'],
  feelsLike: ['Percepita', 'Feels like'],
  gusts: ['Raffiche', 'Gusts'],
  precipitation: ['Precipitazioni', 'Precipitation'],
  rain: ['Pioggia', 'Rain'],
  showers: ['Rovesci', 'Showers'],
  snow: ['Neve', 'Snow'],
  uv: ['Indice UV massimo', 'Maximum UV index'],
  minimum: ['Minima', 'Minimum'],
  maximum: ['Massima', 'Maximum'],
  details: ['Dettagli', 'Details'],
  refresh: ['Aggiorna', 'Refresh'],
  refreshing: ['Aggiornamento...', 'Refreshing...'],
  stale: ['I dati potrebbero non essere aggiornati.', 'The data may be out of date.'],
  addFavorite: ['Aggiungi ai preferiti', 'Add to favourites'],
  removeFavorite: ['Rimuovi dai preferiti', 'Remove from favourites'],
  ...
  close: ['Chiudi', 'Close'],
  infoCopy: [
    'WeatherApp è gratuita, senza pubblicità, account, acquisti o tracciamento. Recenti, preferiti e lingua vengono salvati soltanto in questo browser.',
    'WeatherApp is free, with no ads, accounts, purchases or tracking. Recent cities, favourites and language are stored only in this browser.',
  ],
  sourceCopy: [
    'Ricerche, dati meteo, previsioni ed eventi di sole e luna provengono da Open-Meteo. Icone e immagini atmosferiche sono risorse locali.',
    'Search, weather data, forecasts and sun and moon events come from Open-Meteo. Icons and atmospheric images are local assets.',
  ],
} as const;
...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [CommonModule, FormsModule, WeatherSceneEffectsComponent],
...
export class AppComponent implements OnDestroy {
  ...
  @HostBinding('style.--orb-top') get orbTop(): string | null {
  ...
  get isDataStale(): boolean {
    return !!this.weatherData && Date.now() - this.weatherData.updatedAt > 15 * 60 * 1000;
  }
  t(key: LabelKey): string {
    return LABELS[key][this.languageCode === 'it' ? 0 : 1];
  }
    ...
    if (!city || city.length < 4) {
      ...
      this.errorMessage = !city
        ...
        : this.languageCode === 'it'
          ...
          : 'Enter at least 4 letters to avoid random results.';
      return;
    }
    const selected = this.selectedSuggestion?.label === city ? this.selectedSuggestion : null;
    this.runWeatherRequest(
      selected
        ? this.weatherService.getWeatherForSuggestion(selected)
  ...
  private runWeatherRequest(
    request: ReturnType<WeatherService['getWeather']>,
    refresh: boolean,
  ): void {
    this.searchSubscription?.unsubscribe();
    this.suggestionSubscription?.unsubscribe();
  ...
  onCityKeydown(event: KeyboardEvent): void {
    ...
    if (event.key === 'Enter') {
      ...
      if (this.showSuggestions && suggestion) this.selectCitySuggestion(suggestion);
      else this.getWeather();
      return;
    }
    if (!['ArrowDown', 'ArrowUp'].includes(event.key) || !this.suggestions.length) return;
    event.preventDefault();
  ...
  isSelectedCity(value: string): boolean {
    return (
      this.normalize(this.city) === this.normalize(value) ||
      this.normalize(this.city.split(',')[0]) === this.normalize(value)
    );
  }
  ...
  searchFavorite(favorite: FavoriteCity): void {
    this.city = favorite.label;
    this.selectedSuggestion = favorite;
    this.getWeather();
  }
  ...
  setWeatherView(view: WeatherView): void {
    this.weatherView = view;
  }
  openDay(day: DailyForecast): void {
    this.rememberFocus();
    this.selectedDay = day;
    this.focusDialog('day-details-dialog');
  }
  openHour(hour: HourlyForecast): void {
    this.rememberFocus();
    this.selectedHour = hour;
    this.focusDialog('hour-details-dialog');
  }
  closeDetails(): void {
  ...
  openSettings(): void {
    ...
    this.focusDialog('settings-dialog');
  }
  closeSettings(): void {
  ...
  setLanguageMode(mode: AppLanguageMode): void {
    ...
    this.appLanguageService.save(mode);
    this.languageCode = this.appLanguageService.resolve(mode);
    this.weatherService.setLanguage(this.languageCode);
    this.applyDocumentLanguage();
    this.isSettingsOpen = false;
  ...
  toggleFavorite(): void {
    if (this.weatherData)
      this.favorites = this.favoriteCitiesService.toggle(this.currentFavorite());
  }
  isCurrentFavorite(): boolean {
    return !!this.weatherData && this.favoriteCitiesService.contains(this.currentFavorite());
  }
  ...
  @HostListener('document:keydown.escape') onEscape(): void {
    if (this.selectedDay || this.selectedHour) this.closeDetails();
    else if (this.isSettingsOpen) this.closeSettings();
    else if (this.isInfoPanelOpen) this.closeInfoPanel();
  }
  ...
  private queueCitySuggestions(value: string): void {
    ...
    this.suggestionTimer = setTimeout(() => {
      this.suggestionSubscription = this.weatherService.searchCities(query).subscribe({
        next: (items) => {
          if (this.normalize(this.city) === this.normalize(query)) {
        ...
        error: (error: unknown) => {
          ...
          this.showSuggestions = false;
          this.isSuggesting = false;
          if (error instanceof WeatherConnectionError) this.errorMessage = this.weatherError(error);
        },
      });
    }, 260);
  }
  ...
  private clearDialogFocusTimer(): void {
  ...
  private focusDialog(id: string): void {
    this.clearDialogFocusTimer();
  ...
  private weatherError(error: unknown): string {
    ...
    if (error instanceof WeatherServiceError)
      return this.languageCode === 'it'
        ...
        : 'The weather service is unavailable. Try again shortly.';
    if (error instanceof WeatherNotFoundError)
      return this.languageCode === 'it'
        ? 'Città non trovata. Prova con un nome più specifico.'
        : 'City not found. Try a more specific name.';
    return this.languageCode === 'it'
      ? 'Non è stato possibile aggiornare il meteo.'
      : 'The weather could not be updated.';
  }
  ...
  private getSolarOrbTop(weather: WeatherViewModel): number {
    ...
    const local = this.localIso(weather.timeZone);
    const sunrise = cleanCivilIso(weather.sunRise);
    const sunset = cleanCivilIso(weather.sunSet);
    if (weather.theme === 'sunrise')
      return this.lerp(
        72,
        42,
        this.easeOut(
          this.progress(local, shiftCivilMinutes(sunrise, -30), shiftCivilMinutes(sunrise, 60)),
        ),
      );
    if (weather.theme === 'sunset')
      return this.lerp(
        42,
        72,
        this.easeIn(this.progress(local, shiftCivilMinutes(sunset, -60), sunset)),
      );
    if (weather.theme === 'sunny' || weather.theme === 'partly-cloudy')
      return 68 - Math.sin(Math.PI * this.progress(local, sunrise, sunset)) * 54;
    return weather.theme.includes('night') ? 7 : 18;
  }
  private localIso(zone: string): string {
  ...
  private progress(value: string, start: string, end: string): number {
    const parse = (iso: string) => Date.parse(`${iso}Z`);
    return Math.min(1, Math.max(0, (parse(value) - parse(start)) / (parse(end) - parse(start))));
  }
  private lerp(a: number, b: number, p: number): number {
    return a + (b - a) * p;
  }
  private easeIn(p: number): number {
    return p * p;
  }
  private easeOut(p: number): number {
